...
export class GameUI {
      ...
      <div class="ui-layer">
        <section class="start-screen" id="start-screen">
          <form class="start-card" id="join-form">
            <div class="eyebrow"><span></span> PLAYABLE ALPHA 0.3</div>
  ...
  setAutoFire(enabled: boolean): void { this.autoFire.classList.toggle('active', enabled); const label = this.autoFire.querySelector('b'); if (label) label.textContent = enabled ? 'ON' : 'OFF'; }
  ...
  update(snapshot: WorldSnapshot): PlayerSnapshot | null {
    ...
    if (choices.length > 0 && !self.dead) {
      this.classChoices.innerHTML = choices.map((choice) => {
        const definition = CLASS_DEFINITIONS[choice];
        return `<button data-class-choice="${choice}"><strong>${definition.label}</strong><span>${definition.description}</span><small>LEVEL ${definition.unlockLevel}</small></button>`;
      }).join('');
    }
    ...
    if (self.dead) {
      ...
      this.respawnCountdown.textContent = remaining > 0 ? `Respawn verfügbar in ${(remaining / 1000).toFixed(1)}s` : `Respawn bereit · automatisch in ${Math.max(0, Math.ceil((self.autoRespawnAt - snapshot.serverTime) / 1000))}s`;
    }
  ...
  toast(title: string, message: string, tone: 'normal' | 'danger' | 'success' = 'normal'): void {
    ...
    const heading = document.createElement('strong'); const text = document.createElement('span');
    heading.textContent = title; text.textContent = message; toast.append(heading, text); this.toastContainer.append(toast);
  ...
  private renderLeaderboard(snapshot: WorldSnapshot): void {
    this.leaderboard.innerHTML = `<div class="panel-title">TOP PLAYERS</div>${snapshot.leaderboard.map((entry, index) => `<div class="leader-row ${entry.id === snapshot.selfId ? 'self' : ''}"><b>${index + 1}</b><span>${this.escape(entry.name)}${entry.isBot ? '<em>BOT</em>' : ''}</span><small>${CLASS_DEFINITIONS[entry.playerClass].label} · L${entry.level}</small><strong>${entry.score.toLocaleString('de-DE')}</strong></div>`).join('')}`;
  }
  private renderKillfeed(snapshot: WorldSnapshot): void { const latest = snapshot.killfeed.at(-1)?.id ?? 0; if (latest > this.lastKillEvent) this.lastKillEvent = latest; this.killfeed.innerHTML = snapshot.killfeed.slice(-4).reverse().map((event) => `<div><strong>${this.escape(event.killer)}</strong><span>eliminierte</span><b>${this.escape(event.victim)}</b></div>`).join(''); }
  private renderMinimap(snapshot: WorldSnapshot, self: PlayerSnapshot): void { const context = this.minimap.getContext('2d'); if (!context) return; const { width, height } = this.minimap; context.clearRect(0, 0, width, height); context.fillStyle = 'rgba(7,10,18,.88)'; context.fillRect(0, 0, width, height); context.fillStyle = 'rgba(255,255,255,.13)'; for (const wall of snapshot.walls) context.fillRect(wall.x / GAME.worldWidth * width, wall.y / GAME.worldHeight * height, wall.width / GAME.worldWidth * width, wall.height / GAME.worldHeight * height); for (const player of snapshot.players) { if (player.dead) continue; context.beginPath(); context.fillStyle = player.id === self.id ? '#8c95ff' : '#ef7181'; context.arc(player.position.x / GAME.worldWidth * width, player.position.y / GAME.worldHeight * height, player.id === self.id ? 3.2 : 2, 0, Math.PI * 2); context.fill(); } context.strokeStyle = 'rgba(255,255,255,.18)'; context.strokeRect(0.5, 0.5, width - 1, height - 1); }
  private escape(value: string): string { const node = document.createElement('span'); node.textContent = value; return node.innerHTML; }
  private require<T extends HTMLElement = HTMLElement>(selector: string): T { const element = this.root.querySelector<T>(selector); if (!element) throw new Error(`Missing UI element: ${selector}`); return element; }
}
