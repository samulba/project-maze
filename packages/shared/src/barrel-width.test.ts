...
import { grundbreite } from './barrels';
...
/**
 * Die Rohr-Grundbreite. Aus `apps/client/src/barrel-geometry.ts` hierher
 * gezogen (16.08.), weil ein Lauf-Profil sie skalieren darf und der seitliche
 * Versatz in Rohrbreiten gerechnet wird – eine Breite, die nur der Client
 * kennt, kann kein Profil verschieben und keine Kugel versetzen.
 *
 * Die Regeln sind unverändert Sams C2/C3-Runden und werden hier unverändert
 * weitergeprüft.
 */
describe('grundbreite', () => {
  it('macht jede Klasse dicker als die alte Drei-Töpfe-Regel, außer Sniper', () => {
    for (const id of barrelKlassen) {
      ...
      const height = grundbreite(definition);
  ...
  it('hält Sniper als dünnste Röhre im Spiel – dünner als jede andere Klasse', () => {
    const sniperHeight = grundbreite(CLASS_DEFINITIONS.sniper);
    for (const id of barrelKlassen) {
      if (id === 'sniper') continue;
      expect(grundbreite(CLASS_DEFINITIONS[id]), id).toBeGreaterThanOrEqual(sniperHeight);
    }
  });
  ...
  it('gibt unterschiedlichen Klassen unterschiedliche Breiten (C2: Tank zu Tank)', () => {
    const breiten = new Set(barrelKlassen.map((id) => grundbreite(CLASS_DEFINITIONS[id])));
  ...
  it('bleibt innerhalb einer plausiblen Obergrenze', () => {
    for (const id of barrelKlassen) {
      expect(grundbreite(CLASS_DEFINITIONS[id]), id).toBeLessThanOrEqual(28);
    }
  });

  it('rechnet konkret für Sniper: 12 zu eng, jetzt breiter, aber am dünnsten', () => {
    const height = grundbreite(CLASS_DEFINITIONS.sniper);
