...
export interface ChooseClassMessage {
  type: 'chooseClass';
...
export const CLASS_DEFINITIONS: Record<PlayerClass, ClassDefinition> = {
  ...
  warden: classDef({ id: 'warden', label: 'Warden', description: 'Mehr Drohnen und bessere defensive Kontrolle.', parent: 'drone', unlockLevel: 25, branch: 'control', maxHealth: 116, regen: 2.3, acceleration: 1170, moveSpeed: 238, reload: 0.40, projectileSpeed: 0, projectileLife: 0, damage: 19, projectileRadius: 0, penetration: 0, bodyDamage: 12, barrelCount: 0, barrelSpread: 0, barrelLength: 0, droneCount: 6, droneRespawn: 0.95 }),
  crusher: classDef({ id: 'crusher', label: 'Crusher', description: 'Massiver Rammer mit hoher Standfestigkeit.', parent: 'rammer', unlockLevel: 25, branch: 'impact', maxHealth: 158, regen: 2.8, acceleration: 1370, moveSpeed: 275, reload: 0.48, projectileSpeed: 650, projectileLife: 1.2, damage: 10, projectileRadius: 8, penetration: 14, bodyDamage: 46, barrelCount: 1, barrelSpread: 0, barrelLength: 25, droneCount: 0, droneRespawn: 0 }),
  storm: classDef({ id: 'storm', label: 'Storm', description: 'Vier Läufe erzeugen eine kontrollierbare Kugelwand.', parent: 'twin', unlockLevel: 40, branch: 'rapid', maxHealth: 100, regen: 1.9, acceleration: 1300, moveSpeed: 260, reload: 0.17, projectileSpeed: 820, projectileLife: 1.45, damage: 10, projectileRadius: 6, penetration: 15, bodyDamage: 10, barrelCount: 4, barrelSpread: 0.32, barrelLength: 34, droneCount: 0, droneRespawn: 0 }),
  ...
  overseer: classDef({ id: 'overseer', label: 'Overseer', description: 'Großer Schwarm mit hohem Mikro-Management.', parent: 'warden', unlockLevel: 40, branch: 'control', maxHealth: 120, regen: 2.5, acceleration: 1140, moveSpeed: 232, reload: 0.34, projectileSpeed: 0, projectileLife: 0, damage: 23, projectileRadius: 0, penetration: 0, bodyDamage: 12, barrelCount: 0, barrelSpread: 0, barrelLength: 0, droneCount: 8, droneRespawn: 0.72 }),
  juggernaut: classDef({ id: 'juggernaut', label: 'Juggernaut', description: 'Langsamer, extrem widerstandsfähiger Kollisions-Tank.', parent: 'crusher', unlockLevel: 40, branch: 'impact', maxHealth: 205, regen: 3.4, acceleration: 1180, moveSpeed: 248, reload: 0.58, projectileSpeed: 620, projectileLife: 1.1, damage: 9, projectileRadius: 9, penetration: 14, bodyDamage: 66, barrelCount: 1, barrelSpread: 0, barrelLength: 22, droneCount: 0, droneRespawn: 0 })
};
...
export const UPGRADE_IDS: UpgradeId[] = ['maxHealth', 'regen', 'moveSpeed', 'reload', 'damage', 'projectileSpeed', 'penetration', 'bodyDamage'];
...
export const availableClassChoices = (current: PlayerClass, level: number): PlayerClass[] => (Object.values(CLASS_DEFINITIONS) as ClassDefinition[]).filter((definition) => definition.parent === current && definition.unlockLevel <= level).map((definition) => definition.id);
...
export const classAvailableAtLevel = (playerClass: PlayerClass, level: number): PlayerClass => { let current = CLASS_DEFINITIONS[playerClass]; while (current.unlockLevel > level && current.parent) current = CLASS_DEFINITIONS[current.parent]; return current.id; };
export const GAME = { worldWidth: 6000, worldHeight: 4000, visibleWorldWidth: 1600, visibleWorldHeight: 900, viewRadius: 1100, playerRadius: 22, tickRate: 40, snapshotRate: 20, maxUpgradeLevel: 8, maxLevel: 45, maxPlayers: 40, shapeTargetCount: 220, respawnDelayMs: 2500, autoRespawnDelayMs: 7000, respawnInvulnerabilityMs: 2600 } as const;
